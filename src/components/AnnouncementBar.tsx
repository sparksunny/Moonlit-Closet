...
export const AnnouncementBar: React.FC<AnnouncementBarProps> = ({ 
  ...
  const activeMessages = messages && messages.length > 0 ? messages : MESSAGES;
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <button
          ...
          onClick={() => setCurrentIndex((prev) => (prev - 1 + activeMessages.length) % activeMessages.length)}
        ...
        <div className="flex items-center gap-2">
          {isAdminLoggedIn && onOpenAdmin && (
            <button
              onClick={onOpenAdmin}
              className="hidden md:inline-flex items-center gap-1 text-[10px] uppercase tracking-wider bg-[#3B2A20] text-[#FFFDF9] px-2 py-0.5 rounded-[1px] hover:bg-[#523B2D] transition-colors"
            >
          ...
          <button
            ...
            onClick={() => setCurrentIndex((prev) => (prev + 1) % activeMessages.length)}
            className="hidden sm:inline-flex p-1 hover:text-[#B99A62] transition-colors focus:outline-none"
